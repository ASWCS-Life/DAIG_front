...
반면, 다른 파라미터들인 'total task', 'task size' 그리고 'max_contributer' 는 model.fit() 함수에 적용되는 값이 아닙니다.
...
따라서 작은 수의 K로 설정하면 최종 모델의 accuracy는 올라갈 수 있지만, max contributor(최대 기여자)값이 작아질수록 학습 속도는 저하될 수 있습니다.
반면에, 큰 값의 K로 설정하면 학습 속도는 빨라지지만, accuray 값은 나빠질 수 있습니다.
하지만 자세한 역학관계는 상황에 따라 다릅니다.
...
따라서 max contributor 값이 task size보다 초과하면 그 초과한 수만큼의 기여자는 의미가 없을 것입니다.
...
Finally, click start button, then uploading and creating project will proceed.
...
(모델 정보 및 데이터 정보와는 무관합니다)
DAIG 클라이언트가 데이터를 받으면 텐서플로우의 model.fit()를 통해 분산학습을 시작합니다.
또한 리소스제공자는 언제든지 학습 중단(또는 프로그램의 종료)이 가능합니다.
...
* If you upload incorrect type of model or data, it may cause eniter system malfunction (currently trying to handle as many errors as possible)
* DAIG reads train data at once. so, if train data size is too big (ex. 20GB or more / depends on computer performance), you may not be able to upload total data. upload for bigger data will be supported later.
...
만약 이 프로젝트를 자체적으로 사용해보고 싶다면 https://github.com/ASWCS-Life/DAIG_back 를 방문하여 서버 코드를 확인해주세요.

* 맞지 않는 모델과 데이터 타입의 업로드로 인해 시스템 오작동이 생길 수 있습니다. (현재 최대한 에러들을 제어하고자 하고 있습니다)
* DAIG 시스템은 학습 데이터를 한번에 읽습니다. 따라서 만약 훈련 데이터 사이즈가 너무 크면(예. 20GB 보다 큰 값인 경우 / 컴퓨터 성능에 따라 달라질 수 있습니다), 모든 데이터 업로드가 불가능할 수 있습니다. 더 큰 사이즈의 데이터는 이후 지원예정입니다.
